...
- 用户可随时关闭支付界面
- 用户可在订单中的“待支付”类别中看到未支付的订单
- 用户可点击未支付的订单，重新打开支付界面进行支付
...
- 查询结果界面应当包含如下内容：
  ...
  - 车次信息：
    ...
    - 价格
    - 剩余座位信息：
      - 对于剩余数量>20%的，显示为绿色；<20%的，显示为黄色；无剩余座位的，显示为红色
...
- 在“订单”界面，默认按时间做排序
- 将来的行程（未结束的订单）、已经结束的订单、失败的订单，三者在视觉上应该有所区分
...
- “订单详情”界面应含有如下信息：
  ...
  - 订单号
  - 订单详情：
    - 火车票：车次、始发时间、乘车人、座位号、金额
    - 酒店预订：酒店名称、旅客名、房间类型、住宿时间、金额
    - 火车餐预订：车次、用餐人、用餐时间、菜品、金额
  - 订单操作按钮：
    - 对于“未支付”状态的订单，显示支付按钮
    - 对于“已支付”、“未出行”、“行程中”、“已完成”的订单，显示取消订单按钮
      - 若符合条件，为可点击状态；若不符合条件，未不可点击状态，并在按钮上同时显示不可取消原因。
        - 订单距离行程开始的时间太近（发车前 2 小时不可取消）
        - 订单已经处于行程中、已完成状态
    - 对于“失败”、“已取消”的订单，不显示按钮
...
- 高级搜索：查询酒店名称、城市
- 排序：根据价格、评分排序
- 酒店详情：查看用户评价以及评分、总体评分，住过（预订且未退订）的用户可发表平均并评分
...
- 对于每个用户，其有多少个该酒店的已完成订单（按住宿人数计算），则可以撰写对应条数的评价
...
#### US2.2.1：作为旅客，我想查看火车餐，以便辅助决定如何选择车次

验收标准：

- 在车次查询界面（见 US1.2.1），无论用户有没有预订该车次，都应能查看该车次提供的火车餐
- 对于每个车次，应当有“查看火车餐”按钮，点击后进入“火车餐预订”界面
  - 若该车次提供火车餐（运行时间较长、包含饭点），“查看火车餐”按钮为可以点击状态
  - 若该车次不提供火车餐，“查看火车餐”按钮为不可点击状态，且鼠标悬停时显示不可点餐原因
- 火车餐预订界面应当包含如下信息：
  - 车次
  - 始发时间、到达时间
  - 用餐时间选择：日期+午餐/晚餐
...
- 当天有行程时，在页面上显示通知（无论当前正在浏览哪个部分）

#### US2.3.2：作为乘客，我想查看历史通知，以便于回顾行程

验收标准：

- 当用户点击“个人”页面中的“消息”按钮时，可进入历史消息界面
- 历史消息界面应当包含如下内容：
  - 筛选条件
    - 日期范围
    - 消息类型：订单通知、行程通知
...
- 查询结果界面应当包含如下内容：
  ...
  - 排序选项：按出发时间、按行程时间、按价格
  - 车次信息：
    - 第一程：始发站、始发时间；终到站、终到时间；第二程：始发站、始发时间；终到站、终到时间
    - 中间换乘可用的时间
    - 第一程车次号、第二程车次号
