...
- 火车餐预订界面支持按车次查询、按起始到达站查询（可选，用于点外卖）
- 火车餐预订界面应当包含如下信息：
  - 车次
  - 始发时间、到达时间
  - 用餐时间选择：日期
  - 选择类别：
    - 火车餐 -> 选择时段（午餐、晚餐）
    - 外卖 -> 选择起终点（需后端返回对应车次信息后选择，不选择则默认查询从始发站到终到站的全部站点）
      - 若实现过于麻烦，则可先不实现这部分功能
  - 对于每个可选的**火车餐**：
    - 名称
    - 类别（主食、饮料、零食）
...
- 点击预订列表图标，可以悬浮窗或侧边栏的方式显示预订列表，其中应当包含如下信息：
  ...
  - 提交订单按钮
- 用户应当能从预订列表中移除预订项目（或者勾选）
- 提交订单界面应当可以选择具体的用餐时间段（例如：对于火车上的餐食：12：00-12：30等，对于火车上点的外卖：到达外卖所在车站后的一段时间（不可修改））
- 提交订单后，应进入订单支付流程
- 注：预订列表**不要求**后端保存，但应当在切换页面、刷新页面后仍能保留（参考：Local Storage）
