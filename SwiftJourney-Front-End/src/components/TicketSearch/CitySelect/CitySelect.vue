                ...
                <el-tab-pane label="ABCDEF" name="ABCDEF">
                    <div class="city_name_wrap">
                        <div class="city_name_box" v-for="c in 'ABCDEF'.split('')" :key="c">
                            ...
                            <div class="city_name">
                                <p v-for="(item, index) in partTwoByCharacter(c)" :key="index" @click="handleCityClick(item)">
                                    {{ item.cityName }}
                                </p>
                            </div>
                        </div>
...
function handleCityClick(item: string | Object) {
    // 统一处理字符串和对象类型
    const cityName = typeof item === 'string' ? item : (item as any).cityName;
    emit('handleCityClick', cityName);
}
...
const updateSuggestions = computed(() => {
    ...
    const suggestions = new Set<string>(); // 使用 Set 避免重复
