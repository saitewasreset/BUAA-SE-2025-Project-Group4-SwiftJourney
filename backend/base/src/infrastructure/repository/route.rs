...
impl RouteRepository for RouteRepositoryImpl {
    async fn load(&self) -> Result<Vec<Route>, RepositoryError> {
        ...
        for v in route_id_to_stops.into_values() {
            result.push(
                RouteDataConverter::make_from_do(v).map_err(RepositoryError::ValidationError)?,
            );
        }
    ...
    async fn save_raw(
        ...
        for stop in raw_routes {
            let station_id = station_name_to_id.get(&stop.station).copied().ok_or(
                RepositoryError::InconsistentState(anyhow!("Station not found: {}", stop.station)),
            )?;
