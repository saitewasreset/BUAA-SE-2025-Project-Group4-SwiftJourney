...
where
    R: CityRepository,
{
    pub fn new(city_repository: Arc<R>) -> Self { Self { city_repository } }
}
    ...
    R: CityRepository,
{
    async fn get_city_map(&self) -> Result<HashMap<ProvinceName, Vec<City>>, GeoServiceError> {
        ...
        for city in cities { let province = city.province().clone(); result.entry(province).or_default().push(city); }
        Ok(result)
    }
    ...
    async fn get_city_by_name(&self, name: &str) -> Result<Option<City>, GeoServiceError> {
        ...
        if cities.is_empty() { Ok(None) } else { Ok(Some(cities[0].clone())) }
    }

    async fn add_city(&self, city: City) -> Result<CityId, GeoServiceError> {
        if !self.city_repository.find_by_name(city.name()).await?.is_empty() {
            return Err(GeoServiceError::CityExists(city.name().to_string()));
        }
        let mut city = city; self.city_repository.save(&mut city).await?; Ok(city.get_id().expect("city should have an id after save"))
    }

    async fn remove_city(&self, city: City) -> Result<(), GeoServiceError> {
        self.city_repository.remove(city).await?; Ok(())
    }
    ...
    async fn modify_city(&self, city_id: CityId, city_name: CityName, province: ProvinceName) -> Result<(), GeoServiceError> {
        if let Some(city) = self.city_repository.find(city_id).await? {
            ...
            Ok(())
        } else { Err(GeoServiceError::NoSuchCityId(city_id.into())) }
    }
}
