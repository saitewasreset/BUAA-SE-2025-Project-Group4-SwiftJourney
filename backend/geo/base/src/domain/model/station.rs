...
use shared::domain::{Aggregate, Entity, Identifiable, Identifier};
...
impl Identifiable for Station {
    ...
    fn get_id(&self) -> Option<Self::ID> { self.station_id }
    fn set_id(&mut self, id: Self::ID) { self.station_id = Some(id); }
}
...
impl Station {
    pub fn new(station_id: Option<StationId>, name: String, city_id: CityId) -> Self {
        Station { station_id, name, city_id }
    }

    pub fn name(&self) -> &str { &self.name }
    pub fn city_id(&self) -> CityId { self.city_id }
}
