...
use shared::domain::model::train::Train;
    ...
    SR: StationRepository,
{
    ...
    async fn internal_build_station_name_to_id(
        &self,
    ) -> Result<HashMap<String, StationId>, Box<dyn ApplicationError>> {
    ...
    async fn internal_build_city_name_to_station_name(
        &self,
    ) -> Result<HashMap<String, Vec<String>>, Box<dyn ApplicationError>> {
        self.geo_port
    ...
    fn resolve_station_ids(
        ...
        city_name_to_station_name: &HashMap<String, Vec<String>>,
    ...
    SR: StationRepository,
{
    ...
    async fn query_train(
        ...
        let station_name_to_id = self.internal_build_station_name_to_id().await?;
    ...
    async fn query_direct_trains(
        ...
        let station_name_to_id = self.internal_build_station_name_to_id().await?;
        let city_name_to_station_name = self.internal_build_city_name_to_station_name().await?;

        let from_ids = self.resolve_station_ids(
            &cmd.departure_station,
            &cmd.departure_city,
            &station_name_to_id,
            &city_name_to_station_name,
        )?;
        let to_ids = self.resolve_station_ids(
            &cmd.arrival_station,
            &cmd.arrival_city,
            &station_name_to_id,
            &city_name_to_station_name,
    ...
    SR: StationRepository,
{
    ...
    async fn build_dto(
        &self,
        sch: &shared::domain::model::train_schedule::TrainSchedule,
        train: Train,
        routes: &[shared::domain::model::route::Route],
